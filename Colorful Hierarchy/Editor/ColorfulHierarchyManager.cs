		...
		private static void HierarchyHighlight_OnGUI(int inSelectionID, Rect inSelectionRect)
		{
			...
			if (GO_Label != null)
			{
				...
				if (Label != null && Event.current.type == EventType.Repaint)
				{
					...
					#endregion
					Rect Offset = new Rect(inSelectionRect.position + new Vector2(16f, 0f), inSelectionRect.size);


					#region Background
					if (BKCol.a > 0f)
					{
						Rect BackgroundOffset = new Rect(inSelectionRect.position + new Vector2(16f, 0f), inSelectionRect.size + new Vector2(32f, 0f));
